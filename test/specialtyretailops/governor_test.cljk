(ns specialtyretailops.governor-test
  "Pure unit tests of `specialtyretailops.governor/check` against
  hand-built proposals -- the fast, focused complement to
  `governor-contract-test`'s full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [specialtyretailops.advisor :as adv]
            [specialtyretailops.governor :as gov]
            [specialtyretailops.store :as store]))

(def store-1 {:store-id "store-1" :name "Riverside Specialty Hardware & Home Store" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Specialty Gift Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Northgate Specialty Tools & Hardware Supply" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Specialty Goods Broker Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine specialty retail store coordination"
...
(deftest refund-finalization-content-is-hard-and-permanent
  (testing "a proposal whose rationale touches directly finalizing a refund decision is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "finalized the refund decision after reviewing the customer's complaint"
                          :confidence 0.99)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest replacement-authorization-content-is-hard
  (testing "a proposal touching authorizing a replacement unilaterally is HARD-blocked, same as a refund finalization"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "authorized the replacement unilaterally before store management could review"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest dispute-closure-content-is-hard
  (testing "a proposal touching closing the dispute unilaterally is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :schedule-staffing-operation "store-1")
                          :summary "customer service closed the dispute unilaterally at the counter")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest liability-acceptance-content-is-hard
  (testing "a proposal touching accepting liability for a defect is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "accepted liability for the defect on behalf of the store")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-quality-concern-is-not-scope-excluded
  (testing "flagging observed defective-goods/mis-shipment/product-safety concerns as a QUALITY CONCERN (not a dispute-resolution finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-quality-concern "store-1")
                         :value {:concern "defective batch of hand tools received, mis-shipment from vendor-1"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (defective goods/mis-shipment) is exactly what this op exists to surface"))))
    ...
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "specialty hardware restock" :estimated-cost 480.0 :vendor-id "vendor-1"}
                      {})
