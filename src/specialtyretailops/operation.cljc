(ns specialtyretailops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (SpecialtyRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SpecialtyRetailGovernor (:govern)
  ...
  human store operator/quality coordinator. The approver resumes with
...
(defn build
  ...
  `specialtyretailops.store/Store`).
  opts:
    :advisor      -- a `specialtyretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SpecialtyRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SpecialtyRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
